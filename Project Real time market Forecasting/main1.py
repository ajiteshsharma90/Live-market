...
def add_technical_indicators(data):
    """
    Add technical indicators to the data: SMA, EMA, Bollinger Bands, RSI, and MACD.
    """
    if data.empty or 'Close' not in data.columns:
    ...
    if len(data) < 20:
        st.warning("Not enough data to calculate technical indicators (need at least 20 data points).")
        return data
    ...
    # RSI (Relative Strength Index) with a 14-period window.
...
# Expanded technical indicators list.
...
# ------------------------------
#st_autorefresh(interval=600000, key="real_time_data_refresh")
    ...
    if data.empty:
        ...
        if last_close is None:
            ...
            for indicator in indicators:
                ...
                elif indicator == 'Bollinger Bands' and 'BB_High' in data.columns and 'BB_Low' in data.columns:
                    fig.add_trace(go.Scatter(x=data['Datetime'], y=data['BB_High'],
                                             mode='lines', name='BB High'))
                    fig.add_trace(go.Scatter(x=data['Datetime'], y=data['BB_Low'],
                                             mode='lines', name='BB Low'))
                elif indicator == 'RSI' and 'RSI' in data.columns:
                    fig.add_trace(go.Scatter(x=data['Datetime'], y=data['RSI'],
                                             mode='lines', name='RSI'))
                elif indicator == 'MACD' and 'MACD' in data.columns and 'MACD_Signal' in data.columns:
            ...
            if st.button("Run AI Analysis"):
                st.write("Button pressed: Running AI analysis...")
                with st.spinner("Analyzing the chart, please wait..."):
                    try:
                        # Save chart as a temporary image.
                        st.write("Starting AI analysis...")
                        permanent_file_path = "chart.png"
                        # Save the chart permanently.
                        fig.write_image(permanent_file_path)
                        st.write("Chart saved at:", permanent_file_path)
                        st.image(permanent_file_path, caption="Generated Chart", use_column_width=True)
                        st.write("Reading and encoding image...")

                        # Read the image and encode it to Base64.
                        with open(permanent_file_path, "rb") as image_file:
                        ...
                        #image = PIL.Image.open(tmpfile_path)
                        ...
                        # Debug: Print the full raw response from the AI.
                        st.write("Raw AI Response:", response)

                        # Extract and display the AI analysis results.
                        if response.result:
                            st.write("**AI Analysis Results:**")
                            st.write(response.result)
                        else:
                            st.error("No valid response received from the AI. Please check your prompt or the model status.")
...
st.sidebar.info(
    'This dashboard provides real-time and historical stock data with various technical indicators. '
